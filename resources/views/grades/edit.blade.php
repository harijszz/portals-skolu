...
@section('title', 'Labot atzīmi')

@section('content')
    <h1 class="text-2xl font-bold mb-6">Labot atzīmi</h1>

    <form method="POST" action="{{ route('grades.update', $grade) }}" class="bg-white rounded shadow p-6 max-w-lg">
        @csrf @method('PUT')
        ...
        <div class="mb-4">
            ...
            <select name="subject_id" required
                ...
                @foreach ($subjects as $s)
                    <option value="{{ $s->id }}" {{ old('subject_id', $grade->subject_id) == $s->id ? 'selected' : '' }}>
                        {{ $s->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <input type="number" name="value" step="0.1" min="1" max="10" value="{{ old('value', $grade->value) }}" required
        ...
        <div class="mb-4">
            ...
            <input type="number" name="weight" step="0.1" min="0.1" max="10" value="{{ old('weight', $grade->weight) }}"
        ...
        <div class="mb-4">
            ...
            <input type="date" name="date" value="{{ old('date', $grade->date?->format('Y-m-d')) }}"
                class="w-full border rounded px-3 py-2 text-sm">
        </div>
